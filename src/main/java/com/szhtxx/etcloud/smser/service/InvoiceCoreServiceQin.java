...
import com.szhtxx.etcloud.smser.utils.StringUtilsEx;
...
public class InvoiceCoreServiceQin {
    private static Logger LOG;
    private static BackCalcUtilMethods calcUtilMethods;

    static {
        InvoiceCoreServiceQin.LOG = LoggerFactory.getLogger(InvoiceCoreServiceQin.class);
        InvoiceCoreServiceQin.calcUtilMethods = new BackCalcUtilMethods();
    }
    ...
    public static void openInvoice(BillSubjectDto billSubjectDto, SmruleConfigDto configDto, SmsResultDto smsResultDto) {
        ...
        InvoiceCoreServiceQin.LOG.debug("单据编号[{}]生成发票请求报文 单据:{}，规则配置:{}", billSubjectNo, JSONObject.toJSONString(billSubjectDto), JSONObject.toJSONString(configDto));
        ...
        if (amountIncTax.divide(invLimitAmt, 0, 2).compareTo(new BigDecimal("1000")) > 0) {
            InvoiceCoreServiceQin.LOG.info("单据编号[{}]生成发票请求报文 单据:{}，规则配置:{}", billSubjectNo, JSONObject.toJSONString(billSubjectDto), JSONObject.toJSONString(configDto));
        ...
        try {
            ...
            e.printStackTrace();
            InvoiceCoreServiceQin.LOG.info(e.getMessage());
    ...
    */
        ...
        for (int size = detailDtos.size(), newLineNum = 0, i = 0; i < size + newLineNum; ++i) {
            ...
            if (disLineAmount.compareTo(invLimitAmt) > 0) {
                ...
                if (isLastLine(billSubjectDto, i)) { // 最后一行开票
                    InvoiceCoreServiceQin.LOG.debug("单据编号[{}] 最后一行[{}]开票,金额累计：{},税额误差累计:{}", new Object[]{billNo, i + 1, amountSum, taxAmtErrSum});
                    remark = "最后一行开票";
                    ...
                    InvoiceCoreServiceQin.LOG.debug("单据编号[{}] 超过最大行数 行号[{}]开票,金额累计：{},税额误差累计:{}", new Object[]{billNo, i + 1, amountSum, taxAmtErrSum});
                    remark = "超过最大行数开票";
                    lastIndex = saveInvoince(smsResultDto, configDto, billSubjectDto, lastIndex, i, remark, detailDtos.get(0).getLineNote(), remarkAmountSum);
                    if (splitAmounts != null && !splitAmounts.isEmpty()) {
                        splitAmounts.remove(0);
                    ...
                    if (amountSum.compareTo(invLimitAmt) < 0 && addNextProLineIsExceed(detailDtos, i, amountSum, invLimitAmt)) {
                        ...
                        if ((configDto.getSplitGoodsLine() != 0 || nextLineAmount.compareTo(invLimitAmt) >= 0) && 1 != nextDetails.getSplitSign()) {
                            ...
                            if (splitGoods2.size() > toSplitNum) {
                                ...
                                InvoiceCoreServiceQin.LOG.debug("单据编号[{}] 超过最大行数 行号[{}]开票,金额累计：{},税额误差累计:{}", billNo, i + 1, amountSum, taxAmtErrSum);
                                remark = "金额累计超限开票";
                                lastIndex = saveInvoince(smsResultDto, configDto, billSubjectDto, lastIndex, i, remark, detailDtos.get(0).getLineNote(), remarkAmountSum);
                                if (splitAmounts != null && !splitAmounts.isEmpty()) {
                                    splitAmounts.remove(0);
                    ...
                    if (amountSum.compareTo(invLimitAmt) == 0 || addNextProLineIsExceed(detailDtos, i, amountSum, invLimitAmt)) {
                        InvoiceCoreServiceQin.LOG.debug("单据编号[{}] 金额累计超限 行号[{}]开票,金额累计：{},税额误差累计:{}", billNo, i + 1, amountSum, taxAmtErrSum);
                        remark = "金额累计超限开票";
                        lastIndex = saveInvoince(smsResultDto, configDto, billSubjectDto, lastIndex, i, remark, detailDtos.get(0).getLineNote(), remarkAmountSum);
                        if (splitAmounts != null && !splitAmounts.isEmpty()) {
                            splitAmounts.remove(0);
    ...
    private static InvoiceDetailDto setInvDetailDto(final InvoiceSubjectDto invoiceSubjectDto, final BillDetailDto billDetailDto, final String invSN, final int useLineNote, final String fristLineNote, final boolean mccDistinct, final int lineNum) {
        ...
        if (taxDeduction == null || taxDeduction.compareTo(BigDecimal.ZERO) == 0) {
            ...
            tmpTaxAmt = InvoiceCoreServiceQin.calcUtilMethods.calcTaxAmtByNoTaxMoneyDec(billDetailDto.getAmounts(), null, taxRate, 4);
            ...
            tmpTaxAmt = InvoiceCoreServiceQin.calcUtilMethods.calcTaxAmtByNoTaxMoneyDec(billDetailDto.getAmounts(), taxDeduction, taxRate, 4);
            final BigDecimal invDetailErr = tmpTaxAmt.subtract(invTaxAmt);
            invoiceDETAIL.setInvDetailErr(invDetailErr);
